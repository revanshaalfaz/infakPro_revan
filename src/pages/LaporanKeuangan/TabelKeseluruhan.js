...
import { auth } from "../../Database/Fire"; // Mengimpor Firebase auth
import AuthenticatedUser from "../../Layouts/Authenticated";
import Swal from "sweetalert2";

const TabelKeseluruhan = () => {
  ...
  if (filter === "semua") {
    ...
    combinedData.sort((a, b) => new Date(a.tanggalPengeluaran || a.tanggalPemasukan) - new Date(b.tanggalPengeluaran || b.tanggalPemasukan));
  ...
  const calculateRunningBalance = (items) => {
    ...
    const balanceItems = items.map((item) => {
      ...
      const amount = parseFloat(item.nominalPemasukan || item.nominalPengeluaran);
    ...
    return balanceItems.reverse(); // Membalikkan urutan setelah menghitung saldo
  };

  const allItemsWithBalance = filter === "semua" ? calculateRunningBalance(combinedData) : combinedData;
  const currentItemsWithBalance = allItemsWithBalance.slice(indexOfFirstItem, indexOfLastItem);
      ...
      <div className="min-h-screen w-full bg-white justify-center">
        ...
        <div className="px-[24px] mt-[30px]">
          ...
          <div className="flex justify-center overflow-x-auto">
            <table className=" border-collapse border border-[#000000] table-auto" style={{ marginLeft: "20px", marginRight: "20px" }}>
              <thead>
                <tr>
                  {/* <th className="text-[14px] text-center bg-gray-200 border border-[#000000] text-[#000000] px-4 py-2">Pengguna</th> */}
                  <th className="text-[14px] text-center bg-gray-200 border border-[#000000] text-[#000000] px-4 py-2">Tanggal</th>
                  {filter !== "semua" && <th className="text-[14px] text-center bg-gray-200 border border-[#000000] text-[#000000] px-2 py-2">Kategori</th>}
                  ...
                  {filter === "semua" && <th className="text-[14px] text-center bg-gray-200 border border-[#000000] text-[#000000] px-4 py-2">Saldo Sementara</th>}
                  <th className="text-[14px] text-center bg-gray-200 border border-[#000000] text-[#000000] px-2 py-1">Aksi</th>
                </tr>
              </thead>
              <tbody>
                {currentItemsWithBalance.map((item, index) => {
                  const isIncome = item.hasOwnProperty("nominalPemasukan");
                  const userUid = auth.currentUser?.uid; // UID pengguna yang sedang login
                  console.log("UID user login:", auth.currentUser?.uid);

                  const isOwner = item.uiduser === auth.currentUser?.uid;

                  console.log("Apakah pengguna adalah pemilik data?", isOwner);

                  return (
                    <tr key={item.uid}>
                      {/* <td className="text-center border border-[#000000] text-[#000000]" style={{ fontSize: "12px", padding: "5px" }}>
                        {item.uiduser || item.uiduser}
                      </td> */}
                      <td className="text-center border border-[#000000] text-[#000000]" style={{ fontSize: "12px", padding: "5px" }}>
                      ...
                      {filter === "semua" && (
                        <td className="text-center border border-[#000000] text-[#000000]" style={{ fontSize: "12px", padding: "5px" }}>
                          Rp. {currentItemsWithBalance[index].runningBalance.toLocaleString("id-ID")}
                        </td>
                      )}
                      <td className="text-center border border-[#000000] text-[#000000]" style={{ fontSize: "12px", padding: "5px" }}>
                        {isOwner ? (
          ...
          <div className="flex justify-center mt-4">
            ...
            <button onClick={() => paginate(currentPage + 1)} disabled={currentItemsWithBalance.length < itemsPerPage || currentPage === totalPages} className="px-4 py-2 bg-gray-200 text-gray-600 rounded-md">
              Selanjutnya
            </button>
          </div>
        </div>
...
export default TabelKeseluruhan;
